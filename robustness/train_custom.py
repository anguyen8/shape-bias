...
def tvNorm(img, output, target, tv_beta=1):
    # find gradients
    sel_nodes_shape = target.shape
    ones = ch.ones(sel_nodes_shape).cuda()
    sel_nodes = output[ch.arange(len(target)), target]
    sel_nodes.backward(ones, retain_graph=True)
    grad = img.grad
    # calculate tv norm

    tv_norm = ch.tensor(0.0, requires_grad=True).to('cuda')
    batch = len(grad)
    for i in range(batch):
        img = grad[i]
        row_grad = ch.abs((img[:-1, :] - img[1:, :])).pow(tv_beta).sum()
        col_grad = ch.abs((img[:, :-1] - img[:, 1:])).pow(tv_beta).sum()
        img_tv = row_grad + col_grad
        tv_norm += img_tv/batch

    return tv_norm, grad.norm()

def l2_norm(img, output, target):
    # find gradients
    sel_nodes_shape = target.shape
    ones = ch.ones(sel_nodes_shape).cuda()
    sel_nodes = output[ch.arange(len(target)), target]
    sel_nodes.backward(ones, retain_graph=True)
    grad = img.grad
    # calculate cumulative l2 norm
    # l2_norm = ch.norm(grad, 2, dim=(2,3)).sum()
    l2_norm = grad.norm()
    l2 = Variable(l2_norm, requires_grad=True)
    return l2.cuda()
...
def make_optimizer_and_schedule(args, model, checkpoint, params):
    ...
    *Internal Function* (called directly from train_model)
    ...
    # debug test
    # optimizer = ch.optim.Adam(param_list, args.lr, betas=(0.9, 0.999),
    #                             weight_decay=args.weight_decay)
    # Make schedule
    schedule = None
    if args.step_lr:
...
def _model_loop(args, loop_type, loader, model, opt, epoch, adv, writer):
    ...
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    # if has_custom_train_loss and str(args.custom_train_loss) == 'stander':
    #     train_criterion = ch.nn.CrossEntropyLoss()
    # else:
    #     train_criterion = args.custom_train_loss
    ...
    for i, (inp, target) in iterator:
        ...
        #     print('img', inp.norm(), 'target', target)
        #     plt.show()
        #     ipdb.set_trace()
        ...
        ## tv norm
        if str(args.custom_train_loss) == 'tv' and loop_type == 'train':

            # inp.requires_grad = True
            inp = Variable(inp, requires_grad=True)
            # compute tv norm
            output_grad, _ = model(inp, target=target, make_adv=adv,
                                      **attack_kwargs)
            # model.zero_grad()
            custom_loss, grad = tvNorm(inp, output_grad, target)
            # tv_norm = Variable(tv_norm, requires_grad=True)

            output, final_inp = model(inp, target=target, make_adv=adv,
                                      **attack_kwargs)
            crossEntropy = ch.nn.CrossEntropyLoss()(output, target)
            # loss = crossEntropy + eval(str(args.alpha))*tv_norm
            loss = eval(str(args.alpha)) * custom_loss

        ## tv norm with weighted sum
        elif str(args.custom_train_loss) == 'weighted_tv' and loop_type == 'train':
            # inp.requires_grad = True
            inp = Variable(inp, requires_grad=True)
            # compute tv norm
            output_grad, _ = model(inp, target=target, make_adv=adv,
            ...
            output, final_inp = model(inp, target=target, make_adv=adv,
                                      **attack_kwargs)
            crossEntropy = ch.nn.CrossEntropyLoss()(output, target)
            loss = crossEntropy*(1- eval(str(args.alpha))) + eval(str(args.alpha))*custom_loss

        ## l2 norm
        elif str(args.custom_train_loss) == 'l2' and loop_type == 'train':
            # inp.requires_grad = True
            inp = Variable(inp, requires_grad=True)
            # compute tv norm
            output_grad, _ = model(inp, target=target, make_adv=adv,
                                      **attack_kwargs)
            # opt.zero_grad()
            custom_loss = l2_norm(inp, output_grad, target)
            # tv_norm = Variable(tv_norm, requires_grad=True)

            output, final_inp = model(inp, target=target, make_adv=adv,
                                      **attack_kwargs)
            crossEntropy = ch.nn.CrossEntropyLoss()(output, target)
            loss = crossEntropy + eval(str(args.alpha)) * custom_loss
            # loss = l2 * eval(str(args.alpha))

        ## tv norm test
        elif str(args.custom_train_loss) == 'test_l2' and loop_type == 'train':
            # inp.requires_grad = True
            inp = Variable(inp, requires_grad=True)
            # compute tv norm
            output_grad, _ = model(inp, target=target, make_adv=adv,
            ...
            output, final_inp = model(inp, target=target, make_adv=adv,
                                      **attack_kwargs)
            crossEntropy = ch.nn.CrossEntropyLoss()(output, target)
            loss = custom_loss * eval(str(args.alpha))

        ## Andrew's loss
        elif str(args.custom_train_loss) == 'andrew' and loop_type == 'train':
            output, final_inp = model(inp, target=target, make_adv=adv,
                                      **attack_kwargs)
            crossEntropy = ch.nn.CrossEntropyLoss()(output, target)
            cross_grad = crossEntropy.grad
            ipdb.set_trace()
            loss = crossEntropy + cross_grad * eval(str(args.alpha))
        ...
        ## CrossEntropy
            ...
            #                           **attack_kwargs)
            # # opt.zero_grad()
            # tv_norm, grad = tvNorm(inp, output_grad, target)
        ...
        # ITERATOR
        if str(args.custom_train_loss) in ['tv', 'l2', 'test_l2', 'weighted_tv'] and loop_type == 'train':
            desc = f"{loop_msg} {epoch} |Loss {loss:.3f} |{prec}1 {top1_acc:.3f}| {str(args.custom_train_loss)}:{custom_loss:.3f}|"
        else:
            # desc = f"{loop_msg} {epoch} |Loss {loss:.2f} |{prec}1 {top1_acc:.3f} |tv_norm {tv_norm:.3f} | grad {grad:.3f} ||"
