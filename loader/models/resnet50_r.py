...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, zero_dict=None, keep=False):
    ...
    def forward(self, x, with_latent=False, fake_relu=False, no_relu=False):
        ...
        x = self.layer1(x)
        x = self.zero_out('layer1', x)
        x = self.layer2(x)
        x = self.zero_out('layer2', x)
        x = self.layer3(x)
        x = self.zero_out('layer3', x)
        x = self.layer4(x, fake_relu=fake_relu, no_relu=no_relu)
        x = self.zero_out('layer4', x)
