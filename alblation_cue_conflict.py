from loader.class_loader import alblation_load_model
...
def predict_and_save(data_path, model_name=setting.MODEL, surgery_label= setting.TARGET_LABEL, inter_list=None,
    ...
    if args.robust:
        # MAKE MODEL
        dataset = DATASETS['imagenet'](args.imagenetpath)
        model, checkpoint = make_and_restore_model(arch=args.model, dataset=dataset, resume_path=args.resume)
        transform = transforms.Compose([
        transforms.Resize(256), 
        transforms.CenterCrop(224), 
        transforms.ToTensor(),
        ])
        val_loader = CustomDataset(root_dir='/home/peijie/texture-vs-shape/stimuli/style-transfer-preprocessed-512/', transform=transform)
